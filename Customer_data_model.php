...
class Customer_data_model extends CI_Model {
    ...
    // Option A: deletes all rows (IDs will continue from last number)
   // return $this->db->empty_table('customers'); 
    
    // Option B: Resets the whole table (IDs start back at 1)
     return $this->db->truncate('customers'); 
    }
    ...
    //     // Using CodeIgniter style (assuming CI based on your syntax)
    //     $this->db->where_in('id', $ids);
    //     return $this->db->delete('customers'); // Replace 'customers' with your actual table name
    // }

    public function deleteSelectedRecords($ids) {
        // It is important to check if IDs exist to avoid SQL errors
        if (empty($ids)) {
            return false;
        }

        // This effectively runs: DELETE FROM customers WHERE id IN (1, 5, 8...)
        $this->db->where_in('id', $ids);
        return $this->db->delete('customers'); // Replace 'customers' with your actual table name
    }
...
public function get_by_customerid($id) {
