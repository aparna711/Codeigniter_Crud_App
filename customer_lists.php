...
<div class="container">
    ...
    <form action="<?php echo base_url('index.php/test/displayData'); ?>" method="POST" id="filterForm">
        ...
        <div class="filter-controls">
           ...
           <!--  <div class="filter-group" style="flex-basis: 180px; display: flex; flex-direction: column; gap: 8px;">
                <button type="submit" class="btn-apply">
                    <i class="fas fa-filter"></i> Download data in CSV file
                </button>

                <button type="button" onclick="resetFilters()" class="btn-reset">
                    <i class="fas fa-times-circle"></i> Download data in PDF
                </button>
            </div> -->
...
<div class="filter-group">
    ...
    <div style="position: relative; display: inline-block;">
        <a href="#" class="btn-apply" id="import-btn" onclick="document.getElementById('csv-import').click(); return false;">
            <i class="fas fa-file-upload"></i> 
            <span id="import-text">Import CSV to database</span>
        </a>
        <input type="file" id="csv-import" accept=".csv" style="display:none;" onchange="handleFileSelect(this)">
        
        
        <div id="filename-display" style="margin-top: 5px; font-size: 12px; color: #666; display: none;">
            <i class="fas fa-file-csv"></i> 
            <span id="selected-filename"></span>
            <button type="button" id="upload-btn" class="btn-apply" style="padding: 2px 8px; font-size: 12px; margin-left: 10px;" onclick="uploadCSV()">
                <i class="fas fa-upload"></i> Upload
            </button>
            <button type="button" id="clear-btn" class="btn-clear" style="padding: 2px 8px; font-size: 12px; margin-left: 5px;" onclick="clearFile()">
                Clear
            </button>
        </div>
    </div>
 ...
 <!-- Export CSV -->
    <a href="<?php echo site_url('test/export_csv?' . http_build_query($current_filters)); ?>" 
       ...
       onclick="return confirm('Download <?php echo $total_count; ?> records as CSV?')">
        <i class="fas fa-file-csv"></i> Export CSV from database
    </a>
</div>div
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                                ...
                                echo !empty($interests) ? implode(', ', $interests) : 'None';
                            ?>
                        </td>
...
function confirmDownload(format) {
    var totalCount = <?php echo isset($total_count) ? $total_count : 0; ?>;
    if (totalCount === 0) {
        alert('No records to download!');
        return false;
    }
    return confirm('Download ' + totalCount + ' records as ' + format + '?');
}
...
// function showDeleteConfirmForAllRecords(customerId, customerName) {
//     pendingDeleteId = customerId;
//     // document.getElementById('customerName').textContent = customerName;
//     document.getElementById('deleteModalForAllRecords').style.display = 'flex';
//     document.body.style.overflow = 'hidden';
// }
...
(function() {
    ...
    function clearFile() {
        ...
        document.querySelector('.import-text').textContent = 'Import CSV to database';
...
//         document.getElementById('selected-filename').textContent = selectedFile.name;
//         document.getElementById('filename-display').style.display = 'block';
//         document.getElementById('import-text').textContent = 'Change File';
//         document.getElementById('import-btn').style.background = '#ffc107';
...
//     document.getElementById('csv-import').value = '';
//     document.getElementById('filename-display').style.display = 'none';
//     document.getElementById('import-text').textContent = 'Import CSV to database';
//     document.getElementById('import-btn').style.background = '';
// }
...
//     // Show loading
//     document.getElementById('upload-btn').innerHTML = '<i class="fas fa-spinner fa-spin"></i> Uploading...';
//     document.getElementById('upload-btn').disabled = true;
    
//     let formData = new FormData();
//     formData.append('csv_file', selectedFile);
    
//     // Add current filters (optional - for filtered imports)
...
//     .finally(() => {
//         // Reset button
//         document.getElementById('upload-btn').innerHTML = '<i class="fas fa-upload"></i> Upload';
