...
<head>
    ...
    <link rel="stylesheet"  type="text/css" href="<?php echo base_url('assets/css/download_data.css'); ?>">
    ...
    <link rel="stylesheet"  type="text/css" href="<?php  echo base_url('assets/css/see.css'); ?>">
...
<!-- Edit Modal -->
 <div id="editModal" class="modal">
    <div class="modal-content" id="edit-modal-content">
        <button class="close-btn">&times;</button>
        <div class="modal-header">
            <h2><i class="fas fa-edit"></i> Update Customer</h2>
        </div>
        <form id="editCustomerForm" method="POST">
            ...
            <fieldset>
                ...
                <br><br>
                <label>Phone:</label>
                <input type="tel" id="edit_phone" name="phone" maxlength="10">
                <br><br>
                <label>Age:</label>
            ...
            <fieldset>
                ...
                <input type="radio" id="status_active" name="edit_account_status" value="Active"> Active
                <input type="radio" id="status_inactive" name="edit_account_status" value="Inactive"> Inactive
            </fieldset>

            <fieldset>
                <legend>Product Interest</legend>
                <label><input type="checkbox" name="edit_product_interest[]" value="Product A"> Product A</label><br>
                <label><input type="checkbox" name="edit_product_interest[]" value="Product B"> Product B</label><br>
                <label><input type="checkbox" name="edit_product_interest[]" value="Service C"> Service C</label>
            </fieldset>

            <div style="text-align: center; margin-top: 20px;">
                <button type="submit" id="edit-submit" class="btn-apply-modal">
        ...
        <div class="header-section">
            ...
            <div class="header-actions">
                <button id="openRegisterModal" class="btn-modal">
                 ...
                 <button id="deleteAllRecords" class="btn-modal filter">
...
<div class="table-wrapper">
    ...
    <td><?php //echo html_escape($customer->first_name . ' ' . $customer->last_name); ?></td> -->
                            ...
                            <td>
                                <a href="#" onclick="openEditModal(<?php echo (int)$customer->id; ?>); return false;" class="btn-edit-modal btn-delete-modal" title="Edit">
                                    <i class="fas fa-edit"></i> 
                                </a>
                                 <a href="#" onclick="showDeleteConfirm(<?php echo (int)$customer->id; ?>, '<?php echo htmlspecialchars($customer->first_name, ENT_QUOTES); ?>'); return false;" class="btn-delete" title="Delete">
                            ...
                            <td>
                                <input type="checkbox" class="row-checkbox" value="<?php echo $customer->id; ?>">
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<!-- <div id="deleteConfirmModal" class="modal-overlay" style="display:none;">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <p>You are about to delete <strong>ALL</strong> customer records. This action is permanent and cannot be undone.</p>
            <p>Are you absolutely sure?</p>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn-secondary" id="cancelDelete">Cancel</button>
            <button type="button" class="btn-danger" id="confirmDeleteAll">Yes, Delete Everything</button>
        </div>
    </div>
</div>
 -->
...
<div id="deleteConfirmModal" class="delete-modal" style="display:none;">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="delete-btn" id="confirmDeleteAll">Yes, Delete Everything</button>
        </div>
    </div>
</div>
...
<!-- <div class="modal fade" id="deleteConfirmModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete <strong id="modalSelectedCount">0</strong> selected records? This action cannot be undone.</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                <button type="button" id="confirmDeleteSubmit" class="btn btn-danger">Yes, Delete All</button>
            </div>
        </div>
    </div>
</div> -->
...
<!-- <div class="modal fade" id="deleteConfirmModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Are you sure you want to delete <strong id="modalSelectedCount">0</strong> selected records?</p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                <button type="button" id="confirmDeleteSubmit" class="btn btn-danger">Yes, Delete All</button>
 ...
 <div id="deleteConfirmModal" style="display:none; position:fixed; top:20%; left:30%; background:white; border:1px solid #ccc; padding:20px; z-index:1000;">
    <p>Are you sure you want to delete <span id="modalSelectedCount">0</span> records?</p>
    <button id="confirmDeleteSubmit">Yes</button>
    <button onclick="$('#deleteConfirmModal').hide()">Cancel</button>
</div>
<!-- <td>
                                <input type="checkbox" class="row-checkbox" value="<?php //echo $customer->id; ?>">
                            </td> -->
...
<!-- <div class="delete-modal" id="delete-modal" style="display:none;">
    <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="cancelDelete" data-dismiss="modal">Cancel</button>
                <button type="button" id="confirmDeleteSubmit" class="confirmDeleteAll">Yes, Delete All</button>
            </div>
    </div>
</div> -->
<script>
...
//         $.ajax({
//             url: '<?php // echo base_url("customer/deleteSelected"); ?>', // Path to your controller function
//             method: 'POST',
//             data: { ids: selectedIds }, // Send the array
//             dataType: 'JSON',
...
//                     // Option B: Remove rows without refreshing (better UX)
...
//         if (confirm("Are you sure you want to delete " + selectedIds.length + " items?")) {
//             // Send IDs to your PHP backend via AJAX
//             $.ajax({
//                 url: 'index.php/test/deleteSelectedRecords', // Your PHP file path
//                 method: 'POST',
//                 data: { ids: selectedIds },
//                 success: function(response) {
//                     // Reload page or remove rows from DOM on success
...
$(document).ready(function() {
    ...
    document.querySelectorAll('.close-btn, #cancelDeleteBtn').forEach(btn => {
    ...
    window.onclick = (event) => {
        if (event.target == registerModal || event.target == filterModal || event.target == editModal || event.target == deleteModal) {
            closeAllModals();
        }
    }
...
window.openEditModal = function(customerId) {
    // 1. Reset form to clear previous user's data
    $('#editCustomerForm')[0].reset();
    $('input[name="product_interest[]"]').prop('checked', false);



    // 2. AJAX Fetch
    $.ajax({
        url: '<?php echo base_url("index.php/test/get_customer/"); ?>' + customerId,
        type: 'GET',
        dataType: 'json',
      
        success: function(data) {
    // 3. Fill Text/Hidden Inputs (Using || '' prevents 'null' appearing in boxes)
    ...
    // Ensure these property names match your DB columns exactly!
    // $('#edit_from_membership_date').val(data.membership_from || '');
    // $('#edit)to_membership_date').val(data.membership_to || '');
    ...
    if (data.account_status) {
        $(`input[name="edit_account_status"][value="${data.account_status}"]`).prop('checked', true);
    }
    if (data.gender) {
        $(`input[name="edit_gender"][value="${data.gender}"]`).prop('checked', true);
    }

    // 6. Fill Checkboxes (Crucial for completeness)
    if (data.product_interest) {
        // Handle both Array (from JSON) or String (from comma-separated)
        let interests = Array.isArray(data.product_interest) 
            ? data.product_interest 
            : data.product_interest.split(',');

        interests.forEach(val => {
            let cleanVal = val.toString().replace(/[\[\]"]/g, '').trim();
            $(`input[name="edit_product_interest[]"][value="${cleanVal}"]`).prop('checked', true);
        });
    }
...
error: function() {
            alert("Could not retrieve customer details.");
        }
    });
};

    // 1. REGISTER NEW CUSTOMER
 // $('#deleteAllRecords').on('submit', function(e) {
 //    /* Create a Delete Button */
 //    e.preventDefault();
 //    $.ajax({
 //        url: '<?php //echo base_url("index.php/test/deleteAllRecords"); ?>',
 //        type:'POST',
 //        datatype: formData(),
 //        beforeSend : function(){
 ...
 //        },
 //        failure: function(){
 //            showFlashMessage('error', 'Deletion failed');
 //        }
...
// $('#deleteAllRecords').on('click', function(e) {
//     e.preventDefault();
...
//             url: '<?php // echo base_url("index.php/test/deleteAllRecords"); ?>',
//             type: 'POST',
//             dataType: 'json', // Changed from datatype: formData() which was invalid
//             beforeSend: function() {
//                 $('#deleteAllRecords').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Deleting...');
//             },
//             success: function(response) {
//                 if (response.success) {
//                     showFlashMessage('success', 'All records deleted successfully');
//                     setTimeout(() => location.reload(), 1500);
//                 } else {
//                     showFlashMessage('error', response.message || 'Deletion failed');
//                 }
//             },
//             error: function() {
//                 showFlashMessage('error', 'Server error during deletion');
//             }, 
//             complete: function() {
//                 $('#deleteAllRecords').prop('disabled', false).html('<i class="fas fa-trash"></i> Delete All');
//             }
//         });
//     }
// });
...
//  // 1. "Select All" Toggle
    ...
    //     // Check or uncheck all row checkboxes based on header checkbox
    ...
    // $(document).on('change', '.row-checkbox', function() {
    //     // If one checkbox is unchecked, uncheck the "Select All"
    ...
    //  let selectedIds = []; // Global variable to store IDs temporarily

    // // STEP 1: When user clicks the main "Delete Selected" button
    ...
    //     // Update the count in the modal and show it
    //     $('#modalSelectedCount').text(selectedIds.length);
    //     $('#deleteConfirmModal').modal('show');
    // });

    // STEP 2: When user clicks "Yes, Delete All" inside the Modal
 //    $('#confirmDeleteSubmit').on('click', function() {
 //        const btn = $(this);
 //        btn.prop('disabled', true).text('Deleting...'); // Prevent double clicks
 // deleteSelectedRecords
 //        $.ajax({
 //            url: '<?php //echo base_url("index.php/test/deleteSelected"); ?>',
 //            method: 'POST',
 //            data: { ids: selectedIds },
 //            dataType: 'JSON',
 //            success: function(response) {
 ...
 //                } else {
 //                    alert(response.message);
 //                    btn.prop('disabled', false).text('Yes, Delete All');
 //                }
 //            },
 //            error: function() {
...
$(document).ready(function() {
    ...
    // 3. Show/Hide main Delete button
    function toggleDeleteButton() {
        const selectedCount = $('.row-checkbox:checked').length;
        $('#selectedCount').text(selectedCount);
    ...
    // STEP 1: Open Custom Modal (Replaces .modal('show'))
    $('#deleteSelectedBtn').on('click', function() {
        selectedIds = []; 
        $('.row-checkbox:checked').each(function() {
            selectedIds.push($(this).val());
        ...
        // Update count and show the custom div
        $('#modalSelectedCount').text(selectedIds.length);
        $('#deleteConfirmModal').show(); // Using jQuery show instead of Bootstrap modal
    });

    // STEP 2: Confirm and AJAX Delete
    $('#confirmDeleteSubmit').on('click', function() {
        const btn = $(this);
        btn.prop('disabled', true).text('Deleting...');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    $('#deleteConfirmModal').hide(); // Hide custom div
                    location.reload(); 
                } else {
                    alert(response.message);
...
// A. Open Modal when clicking the main "Delete All" button
$('#deleteAllRecords').on('click', function(e) {
    e.preventDefault();
    $('#deleteConfirmModal').fadeIn(200);
});

// B. Close Modal when clicking "Cancel"
$('#cancelDelete').on('click', function() {
    $('#deleteConfirmModal').fadeOut(200);
});

// C. The Actual AJAX Deletion (Inside the modal)
$('#confirmDeleteAll').on('click', function() {
    $.ajax({
        ...
        dataType: 'json',
        beforeSend: function() {
            $('#confirmDeleteAll').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Processing...');
        },
        success: function(response) {
            if (response.success) {
                ...
                $('#confirmDeleteAll').prop('disabled', false).text('Yes, Delete Everything');
            }
        },
        error: function() {
            showFlashMessage('error', 'Critical server error occurred.');
            $('#deleteConfirmModal').fadeOut(200);
        }
    });
});
...
$(window).on('click', function(event) {
   ...
   // 4. FILTER FORM SUBMISSION (AJAX)
    ...
    $('#from_membership_date, #edit_from_membership_date').attr('min', today);
});
</script>
